...
# for l_factor in [1, 0.1, 0.01]:
#     print(f'Linear Regression test_error  with lambda {l_factor} =',
#           run_linear_regression_on_MNIST(lambda_factor=l_factor))
...
def run_svm_one_vs_rest_on_MNIST(C):
    ...
    train_y[train_y != 0] = 1
    test_y[test_y != 0] = 1
    pred_test_y = one_vs_rest_svm(train_x, train_y, test_x, C=C)
    print(pred_test_y)
    test_error = compute_test_error_svm(test_y, pred_test_y)
    return test_error


# for l_factor in [1, 0.1, 0.01, 0.001]:
#     print('SVM one vs. rest test_error:', run_svm_one_vs_rest_on_MNIST(l_factor))
...
# theta, cost_function_history = softmax_regression(train_pca, train_y, 1, alpha=0.3,
...
# theta, cost_function_history = softmax_regression(train_cube, train_y, 1, alpha=0.3,
#                                                   lambda_factor=1.0e-4, k=10, num_iterations=150)
...
classifier.fit(train_pca10, train_y)
y_pred = classifier.predict(test_pca10)
print(1 - np.mean(y_pred == test_y))